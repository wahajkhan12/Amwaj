...
const Sanitery = ({ navigation }) => {
  ...
  const fetchPackagesAndFees = async () => {
    ...
    const packagesRef = ref(db, 'services/sanitery'); // Change 'Electricity' to the appropriate department
    try {
  ...
  const writeData = async () => {
    ...
    const db = getDatabase();
    const dbRef = ref(db, 'normal/' + user + '/sanitery');
    try {
