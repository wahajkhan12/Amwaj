...
const Electricity = ({ navigation }) => {
  ...
  const fetchPackagesAndFees = async () => {
    ...
    const packagesRef = ref(db, 'services/electricity'); // Change 'Electricity' to the appropriate department
    try {
  ...
  const writeData = async () => {
    ...
    const db = getDatabase();
    const dbRef = ref(db, 'normal/' + user + '/electricity');
    try {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.addButton}><View style={styles.emptySpace}>
          {/*
          <ImageBackground
