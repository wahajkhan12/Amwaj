...
const ElectricDevices = ({ navigation }) => {
  ...
  const fetchPackagesAndFees = async () => {
    ...
    const packagesRef = ref(db, 'services/electric devices'); // Change 'Electricity' to the appropriate department
    try {
  ...
  const writeData = async () => {
    ...
    const db = getDatabase();
    const dbRef = ref(db, 'normal/' + user + '/electric devices');
    try {
