...
const Ac = ({ navigation }) => {
  ...
  const [selectedPackageFee, setSelectedPackageFee] = useState("0");
  ...
  const fetchPackagesAndFees = async () => {
    ...
    const packagesRef = ref(db, 'services/AC'); // Change 'Electricity' to the appropriate department
    try {
  ...
  const writeData = async () => {
    ...
    const db = getDatabase();
    const dbRef = ref(db, 'normal/' + user + '/ac');
    try {
